...
def convert_fulltext(data, w2i, tag2i, ner2i, c2i, update_dict=True, max_len=None, build_chunks=False):
    context_cache = {}
                ...
                if not build_chunks:
                        ...
                        for sent in para:
                            ...
                            if max_len is not None and len(widx) > max_len:
                                ...
                                while curr+max_len < len(widx):
                                    passage_idxs.append((widx[curr:curr+max_len], posidx[curr:curr+max_len], [], []))
                                    curr += max_len
                                passage_idxs.append((widx[curr:], posidx[curr:], [], []))
                            else:
                    ...
                    for para in context:
                        for sent in para:
                            ...
                            ner = sent[3]
                            if update_dict:
                                tmp_w = [w2i.setdefault(w, len(w2i)) for w in words]
                                tmp_pos = [tag2i.setdefault(p, len(tag2i)) for p in pos]
                            else:
                            ...
                            if len(tmp_w) > max_len:
                                ...
                                while curr+max_len < len(tmp_w):
                                    passage_idxs.append((tmp_w[curr:curr+max_len], tmp_pos[curr:curr+max_len], [], []))
                                    curr += max_len
                                assert len(widx) == 0
