...
def test_interactive_aligner(t1, t2, t1_lens, t2_lens):
	...
	success &= test_wrapper('checking if cols of E sum to 1', torch.allclose(torch.sum(E, 1), torch.ones(E.shape[0], E.shape[2])))
	for i in range(len(t1_lens)):
		success &= test_wrapper("checking masking of rows in batch %d of E" % i, 
			torch.all(E[i, t1_lens[i]:, :t2_lens[i]] == 0))
...
def test_self_aligner(t, t_lens):
	...
	success &= test_wrapper('checking shape of E', B.shape[0] == t.shape[0] and
										...
										B.shape[2] == t.shape[1])	
	success &= test_wrapper('checking if cols of B sum to 1', torch.allclose(torch.sum(B, 1), torch.ones(B.shape[0], B.shape[2])))
	for i in range(len(t_lens)):
		success &= test_wrapper("checking masking of rows in batch %d of B" % i, 
			torch.all(B[i, t_lens[i]:, :t_lens[i]] == 0))
...
if __name__ == '__main__':
	...
	# ts1 = [torch.rand(i+2, 2) for i in range(4,-1, -1)]
	...
	ts2 = [torch.arange(0,2*(i+1)).float().view(-1,2) + 1 for i in range(4,-1, -1)]
	# ts2 = [torch.rand(i+1, 2) for i in range(4,-1, -1)]
