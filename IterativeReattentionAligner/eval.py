...
import sacrebleu
from run import build_dicts,convert_data,generate_embeddings,compute_scores
...
def main(args):
    ...
    if args.dicts_dir is not None:
        ...
        w2i, tag2i, ner2i, c2i = build_dicts(eval_data)
    ...
    with torch.no_grad():
        ...
        logger.info("dev average rouge score %.4f, another rouge %.4f, bleu1 score %.4f, bleu4 score %.4f, coco bleu1 %.4f, coco bleu4 %.4f, start acc %.4f, end acc %.4f time=%.2fs" % (avg_rouge, another_rouge_avg, avg_bleu1, avg_bleu4, coco_bleu1, coco_bleu4, dev_start_acc, dev_end_acc, time.time() - start_time))        
